...
gitRepository = "https://github.com/diegogd/sushi-gen"
...
module.exports =
  ...
  downloadTemplate: (callback) ->
    Git.Clone gitRepository, template_git_pwd, { checkoutBranch: "template2" }
    .then (repository) ->
      callback()
  ...
  prepareTemplateFolder: (externalCallback) ->
    async.series [
      (callback) =>
        if !@isTemplateReady()
          console.log "Template folder does not exist %s", template_git_pwd
          spinner = new Multispinner
